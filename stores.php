...
<body class="font-['Poppins']">
  ...
  <!-- Branches Section -->
  <section
    class="w-full flex flex-col items-center justify-center bg-gray-50 py-8 md:py-12 px-4 md:px-8 lg:px-16 xl:px-24 pt-16 md:pt-[100px] mt-16">
    <div class="w-full mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-10 max-w-7xl mx-auto">
        <?php foreach ($branches as $i => $branch): ?>
          ...
          <div class="bg-white rounded-xl shadow p-6 flex flex-col h-full max-w-4xl store-card group hover:shadow-2xl"
            data-aos="zoom-in" data-aos-delay="<?= $randDelay ?>">
            <h3 class="font-extrabold text-base md:text-lg mb-1 text-[#252525]"><?= htmlspecialchars($branch['name']) ?>
            </h3>
            ...
            <a href="<?= htmlspecialchars($branch['map_link']) ?>" target="_blank"
              class="text-[#F01B23] font-bold text-xs md:text-sm mt-auto inline-block">View Map</a>
          </div>
        <?php endforeach; ?>
      </div>
    </div>
