...
<header
  class="w-full bg-white shadow flex items-center justify-between px-4 md:px-8 lg:px-16 xl:px-24 py-4 shadow-lg relative fixed top-0 left-0 z-50"
  data-aos="fade-down">
  <a href="/" class="focus:outline-none"><img src="/images/jmpi-logo.webp" alt="Joshua's Meat Products, Inc."
      class="h-10 md:h-12 lg:h-16" loading="lazy"></a>
  ...
  <div class="hidden xl:flex items-center space-x-4">
    <div class="relative flex items-center group" id="search-group">
      <div class="flex items-center gap-x-1">
        <div class="relative">
          <span
            class="absolute left-2 top-1/2 -translate-y-1/2 text-gray-800 pointer-events-none z-20 flex items-center gap-x-1">
      ...
      <div id="search-results"
        class="absolute left-[-40px] top-full mt-2 w-96 bg-white rounded-xl shadow-lg z-50 hidden"></div>
    </div>
    <span class="mx-2 text-gray-400">|</span>
    <a href="/dealer.php"
      class="px-4 py-2 border-2 border-[#F01B23] text-[#F01B23] rounded-full font-bold text-sm hover:bg-[#F01B23] hover:text-white transition">BE
      A DEALER</a>
  </div>
...
<aside id="sidebar"
  class="fixed top-0 right-0 h-full w-64 bg-white shadow-lg z-50 transform translate-x-full transition-transform duration-300">
  ...
  <nav class="flex flex-col gap-4 px-6 py-6">
    <?php foreach ($nav as $item): ?>
      <a href="<?= htmlspecialchars($item['link']) ?>"
        class="py-2 px-2 rounded hover:bg-red-50 text-xs md:text-sm lg:text-base font-semibold text-gray-700 hover:text-[#F01B23] transition">
        <?= htmlspecialchars($item['name']) ?>
      </a>
    <?php endforeach; ?>
    <a href="/dealer.php"
      class="mt-4 px-4 py-2 border-2 border-[#F01B23] text-[#F01B23] rounded-full font-bold text-center hover:bg-[#F01B23] hover:text-white transition text-xs md:text-sm lg:text-base">BE
      A DEALER</a>
  </nav>
</aside>
